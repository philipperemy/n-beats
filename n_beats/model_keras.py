        ...
        for i in range(self.number_slacks):
            for j in range(self.number_blocks[i]):
                ...
                if self.block_types[j] == 'trend' :
                    ...
                    backcast = Lambda(trend_model, arguments={"is_forecast":False, "length": self.backcast_length})(theta_b)
                    forecast = Lambda(trend_model, arguments={"is_forecast":True, "length": self.forecast_length})(theta_f)
                    X_ = Subtract()([X_, backcast])
                    if i == 0 and j == 0:
                        y_ = forecast
                    else :
                ...
                if self.block_types[j] == 'seasonality' :
                    ...
                    # thetas_b = Dense(self.backcast_length, activation='linear')(d4) # theta_b and theta_f are shared even for seasonality bloc
                    backcast = Lambda(seasonality_model, arguments={"is_forecast": False, "length": self.backcast_length})(theta_b)
                    forecast = Lambda(seasonality_model, arguments={"is_forecast": True, "length": self.forecast_length})(theta_f)
                    X_ = Subtract()([X_, backcast])
                    y_ = Add()([y_, forecast])
    ...
    def trainModel(self):
        ...
        for step in range(nb_epochs):
            ...
            if step % self.plot_results == 0:
                ...
                if getAccuracy(y_test, predictions)[0] < self.best_perf:
                    self.best_perf = getAccuracy(y_test, predictions)[0]
                ...
                plt.scatter(list(range(len(x_y) - self.forecast_length, len(x_y))), predictions[0, :].flatten(), color=['r'] * self.forecast_length)
                ...
                plt.plot(list(range(len(x_y) - self.forecast_length, len(x_y))), validation_predictions[0, :].flatten(), color='r')
                plt.scatter(range(len(x_y)), x_y.flatten(), color=['b'] * self.backcast_length + ['g'] * self.forecast_length)
                plt.scatter(list(range(len(x_y) - self.forecast_length, len(x_y))), validation_predictions[0, :].flatten(), color=['r'] * self.forecast_length)
                plt.legend(['backcast', 'forecast', 'predictions of forecast'])
        ...
        plt.scatter(range(len(x_y)), x_y.flatten(), color=['b'] * self.backcast_length + ['g'] * self.forecast_length)
        plt.scatter(list(range(len(x_y) - self.forecast_length, len(x_y))), predictions[100, :].flatten(), color=['r'] * self.forecast_length)
        plt.legend(['backcast', 'forecast', 'predictions of forecast'])
        ...
        plt.plot(list(range(len(x_y) - self.forecast_length, len(x_y))), validation_predictions[10, :].flatten(), color='r')
        plt.scatter(range(len(x_y)), x_y.flatten(), color=['b'] * self.backcast_length + ['g'] * self.forecast_length)
        plt.scatter(list(range(len(x_y) - self.forecast_length, len(x_y))), validation_predictions[10, :].flatten(), color=['r'] * self.forecast_length)
        plt.legend(['backcast', 'forecast', 'predictions of forecast'])
        plt.savefig('results/test/' + str(nb_epochs)+'.png')

        print(getAccuracy(y_test, validation_predictions))
...
def get_xtrain_circular(backcast_length, forecast_length):
    ...
    headers = True
    filename = 'data/train/'
    file = open(filename + 'Daily-train.csv', "r")
...
def get_xtest_circular(backcast_length, forecast_length):
    ...
    headers = True
    filename = 'data/val/'
    file = open(filename + 'Daily-test.csv', "r")
    reader = csv.reader(file, delimiter = ',')
    for line in reader :
        if not headers :
            x_tl.append(line)
